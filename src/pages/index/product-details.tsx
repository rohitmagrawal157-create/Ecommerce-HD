 ...
 * ══════════════════════════════════════════════════════════════════════
 *  UPDATED:
 *  1. Buy Now button — adds to cart, then navigates to /checkout
 *  2. Related Products — fetches same-category products from API
 *     using fetchProductsByCategory(); falls back to productList
 *  3. isBuyingNow separate loading state passed to ProductActions
 *  4. Body overflow control preserved
  ...
  return (
    <form onSubmit={handleSubmit} className="mb-6 bg-white p-4 rounded-xl border border-gray-200">
      <div className="flex gap-3 mb-3">
        ...
        <select value={String(form.rating)}
...
const CustomerReviews: React.FC<CustomerReviewsProps> = ({
  productId, reviews, onReviewAdded, onReviewDeleted,
}) => {
        ...
        <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          {reviews.map((review, idx) => (
            <ReviewCard key={review.id} review={review} index={idx}
              isAuthenticated={isAuthenticated} onDelete={handleDeleteReview} />
          ))}
        </div>
      ...
      <div className="flex items-start gap-3 px-4 py-3 border-b border-gray-100 bg-green-50">
        ...
        <div>
          <p className="text-sm font-bold text-gray-800">
            Estimated Delivery:{' '}
            <span className="text-green-700">{fmt(d15)} – {fmt(d16)}</span>
          </p>
...
// ── LayoutOne Item shape (must match layout-one.tsx interface Item) ───────────
interface LayoutOneItem {
  id:             number;
  image:          string;
  tag:            string;
  price:          string;   // formatted "₹90" — LayoutOne calls fmtRupee() on this
  name:           string;
  rating?:        number;
  originalPrice?: string;   // MRP formatted string e.g. "₹120"
  discount?:      number;   // percentage e.g. 20
}

// Maps the API Product shape (from products.api.ts) → LayoutOne Item shape.
// API Product: { id, name, price:"₹90", image, tag, rating, originalPrice:"₹120", discount:20 }
// These already match LayoutOne's Item — just need explicit typing + safety.
function toLayoutOneItem(p: Product): LayoutOneItem {
  return {
    ...
    image:         p.image         ?? '',
    tag:           p.tag           ?? '',
    // price is already formatted "₹XX" from mapApiProduct() in products.api.ts
    price:         p.price         ?? '₹0',
    rating:        p.rating,
...
//  1. categoryId known  → GET /api/categories/:id/all-products
//  2. categoryName only → GET /api/products + client-side filter by tag match
//  3. API failure       → fallback to static productList (any category)
//
// All results are mapped to LayoutOneItem before rendering.
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        // The products API returns category as a plain string (e.g. "Portrait Frames").
        // getProductDetailsById maps it to { id: productId, name } — but that id is the
        // product id, NOT a real category id. So we ALWAYS filter by name, never by id.
        const all = await getProducts();
        if (categoryName && categoryName.trim()) {
          const needle = categoryName.toLowerCase().trim();
          // Exact tag match first (e.g. "portrait frames" === "portrait frames")
          const exact   = all.filter(p => (p.tag ?? "").toLowerCase().trim() === needle);
          // Partial match fallback (e.g. "portrait" matches "Portrait Frames")
          ...
          // No category info — show all products (excluding current)
          apiProducts = all;
        }

        if (alive) {
          const items = apiProducts
            .filter(p => p.id !== currentProductId)  // exclude current product
            .slice(0, 4)                              // max 4 cards
            .map(toLayoutOneItem);                    // convert to LayoutOne shape
          setRelated(items);
        }
      } catch (err) {
        console.warn('[RelatedProducts] API failed — using static fallback', err);
        if (alive) {
          // ③ Static fallback: use productList data
          const fallback = (productList as any[])
            .filter((p: any) => {
              if (p.id === currentProductId) return false;
              // Try to match category if we have a name
              if (categoryName && p.tag) {
                return String(p.tag).toLowerCase().includes(categoryName.toLowerCase());
              }
              return true;
            })
            .slice(0, 4)
            .map((p: any): LayoutOneItem => ({
              id:            p.id,
              name:          p.name          ?? '',
              image:         p.image         ?? '',
              tag:           p.tag           ?? '',
    ...
    <div className="s-py-50-100">
      <div className="container-fluid">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-[1720px] mx-auto">
          {related.map(item => (
            // LayoutOne receives a properly typed LayoutOneItem — no "as any" needed
...
export default function ProductDetails() {
  ...
  const [isBuyingNow]    = useState(false);   // ← NEW
  ...
  // Category info for related products.
  // getProductDetailsById maps d.category (string) → { id: productId, name }
  // getProductById / fallback stores category as  (plain string).
  // We must safely handle both shapes to avoid [object Object] in the UI.
  const rawCategory  = (product as any)?.category;
  const categoryId: number | undefined =
    rawCategory && typeof rawCategory === 'object' ? (rawCategory as any).id as number : undefined;
  const categoryName: string | undefined =
    rawCategory && typeof rawCategory === 'object' && (rawCategory as any).name
      ? String((rawCategory as any).name)
      : typeof rawCategory === 'string' && rawCategory
        ? rawCategory
        : (product as any)?.tag
          ? String((product as any).tag)
          : undefined;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // ── Attempt 1: detailed endpoint /api/products/:id/details ──────────
        console.log('[ProductDetails] Fetching details for id:', parsedId);
        const details = await getProductDetailsById(parsedId);
        console.log('[ProductDetails] getProductDetailsById result:', details);
        if (alive && details && (details.name || details.price)) {
          console.log('[ProductDetails] ✅ Using detailed product data');
          setProduct(details as any);
          return;
        }
        console.warn('[ProductDetails] ⚠️ getProductDetailsById returned null/empty, trying fallback...');

        // ── Attempt 2: list endpoint /api/products — find by id ─────────────
        // getProductById returns Product shape: { id, name, price:"₹90", image, tag }
        // We need to normalise it to ProductDetailsType shape before setting.
        const basicProduct = await getProductById(parsedId);
        if (alive && basicProduct) {
          // Parse price from formatted string "₹90" → number 90
          const priceNum = parseFloat(
            String((basicProduct as any).price ?? '0').replace(/[^0-9.]/g, '')
          );
          const origNum = (basicProduct as any).originalPrice
            ? parseFloat(String((basicProduct as any).originalPrice).replace(/[^0-9.]/g, ''))
            : undefined;
          ...
          const normalised: any = {
            ...
            category:           (basicProduct as any).tag
                                  ? { id: 0, name: String((basicProduct as any).tag) }
                                  : undefined,
            // images: wrap single image string into array
            images:             basicProduct.image ? [basicProduct.image] : [],
            variants:           (basicProduct as any).variants ?? [],
            tag:                (basicProduct as any).tag,
          };
        ...
        if (alive && fallbackProduct) {
          ...
          setProduct({
            ...
            variants: [], category: fb.tag ? { id: 0, name: fb.tag } : undefined,
            tag: fb.tag,
          } as any);
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (alive && Array.isArray(rv) && rv.length > 0) setReviews(rv as any);
      } catch {}
  ...
  return (
    <>
      ...
      {toast && (
        <Toast type={toast.type} message={toast.message} onClose={dismissToast} duration={toast.duration} />
      )}
        ...
        <div className="container-fluid">
          <div className="max-w-[1720px] mx-auto flex flex-col lg:flex-row gap-10">
            ...
            <div className="lg:max-w-[635px] w-full min-h-[700px] flex flex-col gap-4">
              ...
              {/* ① Header: name, rating, price, wishlist */}
              <ProductHeader
                name={productName}
              ...
              <p className="text-sm font-medium flex items-center gap-1.5 mt-4 mb-3">
                ...
                Want to buy in bulk?{' '}
                <a
                  href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(`I want to buy ${productName} in bulk.`)}`}
