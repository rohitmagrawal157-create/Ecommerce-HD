...
// ══════════════════════════════════════════════════════════════════════
//  FIXES & IMPROVEMENTS
//  FIX-1  "Save Billing Details" now POSTs to /api/addresses so data
//         is actually persisted in the DB, not just localStorage.
//         localStorage is kept as offline fallback only.
//  FIX-2  Saved addresses load from /api/addresses (GET) with Bearer
//         token. Selecting one pre-fills the form instantly.
//  FIX-3  Edit address → PUT /api/addresses/:id (updates DB row).
//  FIX-4  Delete address → DELETE /api/addresses/:id.
//  FIX-5  Place Order POSTs full order payload to /api/orders including
//         billing, cart lines, shipping, payment method, total.
//  FIX-6  COD flow also hits /api/orders (not a fake timeout).
//  FIX-7  Cart loaded from getCheckout() using Bearer + SessionId.
//  FIX-8  Auth guard: unauthenticated users bounce to /login.
//  FIX-9  "Extra ${currencySymbol}50" typo fixed — was mixing $ and ₹.
//  FIX-10 key in order summary uses line.id (cart_id) not line.product.id.
//  FIX-11 Shipping costs switched to ₹ (₹0 / ₹99 / ₹149).
//  FIX-12 City dropdown updated to Indian cities.
//  FIX-13 Razorpay verify-payment payload now includes order_id too.
//  FIX-14 Form state + API errors surfaced clearly to user.
//  FIX-15 Full UI redesign — brand-consistent, refined, luxury feel.
// ══════════════════════════════════════════════════════════════════════
...
const BRAND2 = 'linear-gradient(135deg,#2563EB 0%,#06B6D4 100%)';
const PRI    = '#5B4FBE';
...
/** Auth-aware headers for apiClient calls */
...
interface SavedAddress extends BillingInfo {
  id: number;
}
...
export default function Checkout() {
  ...
  useEffect(() => {
    ...
    // Pre-fill from localStorage (offline fallback)
  ...
  const loadAddresses = useCallback(async () => {
    ...
    try {
      ...
      if (alive.current) setAddrLoading(false);
    }
  }, [isAuth]);
  ...
  // FIX-1: Save billing → POST /api/addresses (writes to DB)
  async function handleSaveBilling() {
    ...
    try {
      if (selectedAddrId) {
        // FIX-3: update existing address in DB
        await apiClient.put(`/api/addresses/${selectedAddrId}`, payload, {
          headers: authHeaders(),
        } as any);
        setSaveMsg({ type: 'ok', text: '✅ Address updated in your account.' });
      } else {
        // FIX-1: create new address in DB
      ...
      const msg = e?.response?.data?.message ?? e?.message ?? 'Save failed. Please try again.';
  ...
  function buildAddressPayload(b: BillingInfo) {
    return {
      ...
      mobile:    b.phone,
      city:      b.city,
      state:     b.city,           // use city as state fallback
      pincode:   b.pincode,
  ...
  // FIX-5: Place order → POST /api/orders with full payload
  ...
  async function handlePlaceOrder() {
    ...
    try {
      if (paymentMethod === 'card') {
        ...
        } catch { /* non-fatal — proceed without order_id */ }
        ...
        await new Promise<void>((resolve, reject) => {
          const opts: any = {
            ...
            handler:  async (response: any) => {
              try {
                // FIX-13: verify payment on server
                ...
                // FIX-5: persist order in DB after payment verified
        ...
        // FIX-6: COD also persists order in DB
        ...
        <div className="container mx-auto px-4 max-w-[1220px]">
          ...
          {!cartLoading && !cartError && cart.lines.length === 0 && (
            <div className="text-center py-20 bg-white rounded-2xl shadow-sm border border-gray-100">
              ...
              <Link to="/shop" className="inline-block mt-3 text-sm font-semibold text-[#5B4FBE] hover:underline">
                Continue Shopping →
              </Link>
            </div>
          )}
            ...
            <div className="grid lg:grid-cols-[1fr_420px] gap-8">
              ...
              <div className="space-y-6" data-aos="fade-up">
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8">
                  ...
                  {addresses.length > 0 && addrMode === 'view' && (
                    <div className="mb-6">
                          ...
                          <div key={addr.id}
                            ...
                            className={`p-3 rounded-xl border cursor-pointer transition text-[13px] flex items-start justify-between gap-3 ${
                              selectedAddrId === addr.id
                                ? 'border-[#5B4FBE] bg-[#f3f1ff]'
                                : 'border-gray-200 hover:border-[#5B4FBE]/40 bg-gray-50'
                            }`}>
                            <div className="flex items-center gap-2.5 flex-1 min-w-0">
                              <div className={`w-4 h-4 rounded-full border-2 flex-shrink-0 ${
                                selectedAddrId === addr.id ? 'border-[#5B4FBE] bg-[#5B4FBE]' : 'border-gray-300'
                              }`} />
                            ...
                            <div className="flex gap-1 flex-shrink-0">
                              <button onClick={e => { e.stopPropagation(); selectAddress(addr.id); setAddrMode('edit'); }}
                                className="text-[11px] px-2 py-1 rounded bg-gray-200 text-gray-600 hover:bg-[#5B4FBE] hover:text-white transition font-semibold">
                                Edit
                              </button>
                              <button onClick={e => { e.stopPropagation(); handleDeleteAddress(addr.id); }}
                                className="text-[11px] px-2 py-1 rounded bg-red-100 text-red-600 hover:bg-red-500 hover:text-white transition font-semibold">
                                Del
                              </button>
                            </div>
                          </div>
                      ...
                      {addrMsg && (
                        <p className={`text-[12px] mt-2 font-medium ${addrMsg.type === 'ok' ? 'text-green-600' : 'text-red-500'}`}>
                          {addrMsg.text}
                        </p>
                      )}
                  ...
                  {addresses.length === 0 && !addrLoading && (
                    <p className="text-[13px] text-gray-400 mb-4">No saved addresses yet. Fill in the form below to add one.</p>
                  )}
                  ...
                  <div className="grid gap-5">
                    ...
                    <FormField label="Pincode" required error={formErrors.pincode}>
                      ...
                      {pincodeMsg && (
                        <p className={`text-[12px] mt-1.5 font-medium ${pincodeMsg.includes('✅') ? 'text-green-600' : 'text-red-500'}`}>
                          {pincodeMsg}
                        </p>
                      )}
                    </FormField>
                  ...
                  <div className="mt-6 pt-5 border-t border-gray-100 flex items-center gap-4 flex-wrap">
                    ...
                    {saveMsg && (
                      <p className={`text-[13px] font-semibold ${saveMsg.type === 'ok' ? 'text-green-600' : 'text-red-500'}`}>
                        {saveMsg.text}
                      </p>
                    )}
                  </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6" data-aos="fade-up" data-aos-delay="50">
                  ...
                  <div className="space-y-3">
                    {([
                      ...
                      <label key={opt.value}
                        className={`flex items-center justify-between cursor-pointer p-4 border rounded-xl transition ${
                          shippingMethod === opt.value
                            ? 'border-[#5B4FBE] bg-[#f3f1ff]'
                            : 'border-gray-200 hover:border-[#5B4FBE]/40'
                        }`}>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6" data-aos="fade-up" data-aos-delay="80">
                  ...
                  <div className="space-y-3">
                    <label className={`flex items-center gap-3 cursor-pointer p-4 border rounded-xl transition ${
                      paymentMethod === 'card' ? 'border-[#5B4FBE] bg-[#f3f1ff]' : 'border-gray-200 hover:border-[#5B4FBE]/40'
                    }`}>
                    ...
                    <label className={`flex items-center gap-3 cursor-pointer p-4 border rounded-xl transition ${
                      paymentMethod === 'cod' ? 'border-[#5B4FBE] bg-[#f3f1ff]' : 'border-gray-200 hover:border-[#5B4FBE]/40'
                    }`}>
                    ...
                    <label className="flex items-start gap-2.5 cursor-pointer mb-4">
                      ...
                      <span className="text-[12px] text-gray-500">
                        ...
                        <Link to="/terms-and-conditions"
                          className="text-[#5B4FBE] hover:underline font-semibold">
                          Terms & Conditions
                        </Link>
                      </span>
                    </label>
