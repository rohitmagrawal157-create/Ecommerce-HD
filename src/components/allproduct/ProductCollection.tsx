...
// @ts-nocheck
// ══════════════════════════════════════════════════════════════════════
//  PRODUCT COLLECTION v4 — Exact LayoutOne card design + Mobile 70/30
//
//  MATCHES layout-one.tsx exactly:
//  ✦ Same hover overlay: Wishlist / Add Cart / Quick View (3 buttons)
//  ✦ Same staggered entrance (0ms / 58ms / 116ms delay)
//  ✦ Same gradient fill on overlay button hover (::before trick)
//  ✦ Same infPop heart animation on toggle
//  ✦ Same bottom accent bar scaleX on card hover
//  ✦ Same gradient price text (.inf-price equivalent)
//  ✦ Same tag badge (top-left corner) + discount badge below it
//  ✦ Same vignette over image
//  ✦ Same toast (green success / red error) with fade in/out
//  ✦ Quick View modal via createPortal at z-99999
//  ✦ MOBILE: card = horizontal row — image 45% width / info 55%
//            (visually image dominates ~70% of height perception)
//  ✦ DESKTOP: vertical card, 4 cols → 3 cols (≤1199px)
//  ✦ Infinite scroll via IntersectionObserver
//  ✦ Skeleton loaders (match card layout on mobile/desktop)
//  ✦ All API: addToCart, toggleWishlist, isWishlisted, notifyWishlistChanged
// ══════════════════════════════════════════════════════════════════════
...
// Brand tokens — identical to layout-one.tsx B object
const B = {
...
// Tag colour map — same as layout-one.tsx
...
// ─── CSS — replicates every animation from layout-one.tsx ────────────────────
const STYLE_ID = 'pc-v4-styles'
...
const STYLES = `
  ...
  /* ── Keyframes — same names as layout-one where possible ── */
  ...
  .pc4-title {
    font-family:${FONT_HEAD}; font-size:clamp(22px,3.5vw,36px); font-weight:800;
    color:#0f0f13; line-height:1.12; margin:0 0 10px; letter-spacing:-.02em;
  }
  .pc4-sub { font-family:${FONT_BODY}; font-size:14px; color:#8b8b9e; max-width:380px; margin:0 auto; }

  /* ══ GRID ══ */
  .pc4-grid { display:grid; grid-template-columns:repeat(4,1fr); gap:16px; }
  @media (max-width:1199px) { .pc4-grid { grid-template-columns:repeat(3,1fr); } }
  /* Mobile: single column — cards are horizontal rows */
  @media (max-width:767px)  { .pc4-grid { grid-template-columns:1fr; gap:10px; } }

  /* ══ CARD — Desktop: vertical (same as layout-one .inf-cw) ══ */
  .pc4-card {
  ...
  /* ══ MOBILE CARD: horizontal — image left (45% w), info right (55% w) ══
     Image fills full card height → visually ~70% of card area perception
     Info is compact, centered vertically                                   */
  @media (max-width:767px) {
    .pc4-card     { flex-direction:row; height:148px; border-radius:10px; }
    .pc4-img-wrap {
      width:45% !important; aspect-ratio:unset !important;
      height:100% !important; border-radius:10px 0 0 10px !important; flex-shrink:0;
    }
    .pc4-info     { flex:1; padding:10px 11px !important; justify-content:center; gap:4px !important; }
    .pc4-overlay  { display:none !important; }
    .pc4-accent-bar {
      inset:unset !important; right:0 !important; top:0 !important;
      bottom:0 !important; left:unset !important;
      width:2px !important; height:100% !important;
      border-radius:0 10px 10px 0 !important;
    }
    .pc4-tag-badge { display:none !important; }
    .pc4-low       { display:none !important; }
    .pc4-stars-row { display:none !important; }
    .pc4-save      { display:none !important; }
    .pc4-name  { font-size:12px !important; -webkit-line-clamp:2 !important; }
    .pc4-price { font-size:14px !important; }
    .pc4-heart { width:24px !important; height:24px !important; top:6px !important; right:6px !important; }
    .pc4-disc-badge { top:4px !important; font-size:7px !important; padding:2px 6px !important; left:0 !important; }
  }

  /* ── Image wrap ── */
  .pc4-img-wrap {
    position:relative; aspect-ratio:1/1; overflow:hidden;
    background:${B.bgSoft}; flex-shrink:0; border-radius:8px 8px 0 0;
  }
  /* Image zoom — same as layout-one .inf-pi */
  .pc4-img {
  ...
  /* Vignette */
  .pc4-vignette {
    position:absolute; inset-x:0; bottom:0; height:46%;
    background:linear-gradient(to top,rgba(0,0,0,.34),transparent); pointer-events:none;
  }

  /* Tag badge */
  .pc4-tag-badge {
    position:absolute; top:0; left:0; z-index:20;
    font-family:${FONT_BODY}; font-size:9px; font-weight:800;
  ...
  /* Discount badge */
  .pc4-disc-badge {
    position:absolute; top:26px; left:0; z-index:20;
    font-family:${FONT_BODY}; font-size:8px; font-weight:800;
  ...
  /* ══ HOVER OVERLAY — same as layout-one .inf-ov ══ */
  .pc4-overlay {
  ...
  /* Overlay button — same as layout-one .inf-ob */
  .pc4-ob {
  ...
  /* Gradient fill on hover — same as layout-one .inf-ob::before */
  .pc4-ob::before {
    content:''; position:absolute; inset:0;
    background:${B.brandGrad}; opacity:0; transition:opacity .22s ease; z-index:0;
  }
  ...
  /* Toast */
  .pc4-toast {
    position:absolute; top:0; inset-x:0; z-index:50;
    font-family:${FONT_BODY}; font-size:9px; font-weight:700; letter-spacing:.12em;
    text-transform:uppercase; text-align:center; padding:6px; line-height:1;
    pointer-events:none; user-select:none; color:#fff;
  }

  /* ── Info zone ── */
  .pc4-info { padding:12px 12px 14px; display:flex; flex-direction:column; gap:0; flex:1; }
  .pc4-cat {
    font-family:${FONT_BODY}; font-size:9px; font-weight:700; color:${B.purple};
    text-transform:uppercase; letter-spacing:.1em;
    white-space:nowrap; overflow:hidden; text-overflow:ellipsis; margin-bottom:4px;
  }
  .pc4-name-link { display:block; margin-bottom:8px; text-decoration:none; }
  .pc4-name {
    font-family:${FONT_BODY}; font-size:14px; font-weight:500; line-height:1.45; color:${B.body};
    display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical;
    overflow:hidden; margin:0; transition:color .18s ease;
  }
  .pc4-name:hover { color:${B.purple}; }

  .pc4-stars-row { display:flex; align-items:center; gap:3px; margin-bottom:10px; }
  .pc4-rev-count { font-family:${FONT_BODY}; font-size:11px; color:${B.faint}; line-height:1; }
  ...
  /* Gradient price — same as layout-one .inf-price */
  .pc4-price {
    font-family:${FONT_BODY}; font-size:17px; font-weight:700; line-height:1;
    background:${B.brandGrad};
    -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text;
  }
  /* MRP strikethrough — same as layout-one .inf-mrp */
  .pc4-orig { text-decoration:line-through; color:${B.faint}; font-family:${FONT_BODY}; font-size:12px; font-weight:400; line-height:1; }
  /* Discount pill — same as layout-one .inf-disc */
  .pc4-disc-pill {
    ...
    background:${B.discGrad}; flex-shrink:0;
  }
  ...
  .pc4-cart-icon {
    ...
    display:flex; align-items:center; justify-content:center;
    border:1px solid ${B.border}; background:transparent; cursor:pointer; transition:all .2s ease;
  }
  .pc4-cart-icon.added { background:${B.green}; border-color:${B.green}; }
  .pc4-cart-icon:disabled { opacity:.5; cursor:not-allowed; }
  @media (max-width:767px) { .pc4-cart-icon { display:none; } }

  .pc4-save { margin-top:5px; font-family:${FONT_BODY}; font-size:10px; font-weight:600; color:#16A34A; line-height:1; }
  .pc4-oos  { font-family:${FONT_BODY}; font-size:10px; color:#dc2626; font-weight:600; margin-top:4px; }

  /* Mobile cart button */
  .pc4-mob-cart { display:none; }
  @media (max-width:767px) {
    .pc4-mob-cart {
    ...
    .pc4-mob-cart:hover:not(:disabled) { background:${B.brandGrad}; color:#fff; border-color:transparent; }
    .pc4-mob-cart.added { background:${B.green}; color:#fff; border-color:${B.green}; }
    .pc4-mob-cart:disabled { opacity:.6; cursor:not-allowed; }
  }

  /* Bottom accent bar — same as layout-one .inf-bar */
  .pc4-accent-bar {
    position:absolute; inset-x:0; bottom:0; height:2px;
    background:${B.brandGrad}; border-radius:0 0 8px 8px; pointer-events:none;
  ...
  /* ── Skeleton ── */
  .pc4-skel {
    background:linear-gradient(90deg,#ececf0 25%,#e0e0e8 50%,#ececf0 75%);
    background-size:300% 100%; animation:pc4Shim 1.5s infinite linear; border-radius:5px;
  }
  .pc4-skel-card { background:#fff; border:1px solid #ebebf0; border-radius:8px; overflow:hidden; }
  @media (max-width:767px) {
    .pc4-skel-card { display:flex; flex-direction:row; height:148px; border-radius:10px; }
    .pc4-skel-img  { width:45% !important; aspect-ratio:unset !important; height:100% !important; border-radius:10px 0 0 10px; flex-shrink:0; }
    .pc4-skel-info { flex:1; padding:10px 11px; display:flex; flex-direction:column; gap:7px; justify-content:center; }
  }
  ...
  .pc4-spinner-wrap {
    ...
    padding:28px 0; font-family:${FONT_BODY}; font-size:13px; color:#8b8b9e;
  }
  .pc4-spinner {
    width:20px; height:20px; border:2px solid rgba(91,79,190,.2);
    border-top-color:${B.purple}; border-radius:50%; animation:pc4Spin .7s linear infinite;
  }

  /* End of list */
  .pc4-end { text-align:center; padding:24px 0 0; font-family:${FONT_BODY}; font-size:13px; color:#8b8b9e; }
  ...
  .pc4-err {
    ...
    padding:20px 24px; text-align:center; font-family:${FONT_BODY};
    color:#dc2626; font-size:14px; margin-bottom:16px;
  }
  .pc4-retry {
    margin-top:12px; padding:8px 20px; background:#dc2626; color:#fff;
    border:none; border-radius:7px; font-family:${FONT_BODY}; font-size:13px; font-weight:700; cursor:pointer;
  }
  ...
  .pc4-qv-overlay {
    ...
    display:flex; align-items:center; justify-content:center; padding:16px; font-family:${FONT_BODY};
  }
  .pc4-qv-modal {
  ...
  @media (prefers-reduced-motion:reduce) {
    .pc4-card,.pc4-img,.pc4-overlay { transition:none; }
  }
`
...
function Stars({ rating }: { rating: number }) {
  ...
  return (
    <div style={{ display:'flex', gap:2 }}>
      {[1,2,3,4,5].map(s => (
        <svg key={s} width="11" height="11" viewBox="0 0 24 24" fill={s<=f ? B.yellow : '#D1D5DB'}>
...
function SkeletonCard() {
  return (
    <div className="pc4-skel-card">
      <div className="pc4-skel pc4-skel-img" style={{ aspectRatio:'1/1' }} />
      <div className="pc4-skel-info" style={{ padding:'12px', display:'flex', flexDirection:'column', gap:7 }}>
        <div className="pc4-skel" style={{ height:9,  width:'40%' }} />
        <div className="pc4-skel" style={{ height:13, width:'90%' }} />
        <div className="pc4-skel" style={{ height:12, width:'65%' }} />
        <div className="pc4-skel" style={{ height:16, width:'48%' }} />
      </div>
    </div>
  )
}
...
function QuickViewModal({ product, wished, busy, cartAdded, onClose, onWish, onCart }: {
  ...
  const S: React.CSSProperties = { fontFamily: FONT_BODY }
  ...
  return createPortal(
    <div className="pc4-qv-overlay" onClick={onClose}>
      <div className="pc4-qv-modal" onClick={e=>e.stopPropagation()}>
        ...
        {/* Left image */}
        <div className="pc4-qv-left" style={{
          width:'45%',minWidth:'45%',flexShrink:0,background:'#F9FAFB',
          overflow:'hidden',borderRadius:'16px 0 0 16px',position:'relative',
        }}>
          ...
          />
          <span style={{position:'absolute',top:14,left:14,background:tc,color:'#fff',
            fontSize:9,fontWeight:800,letterSpacing:'.12em',textTransform:'uppercase',
            padding:'4px 10px',borderRadius:20,boxShadow:'0 2px 8px rgba(0,0,0,.18)',...S}}>
            {tag}
          </span>
          {disc>0&&<span style={{position:'absolute',top:42,left:14,background:B.discGrad,color:'#fff',
            fontSize:9,fontWeight:800,letterSpacing:'.1em',textTransform:'uppercase',
            padding:'4px 10px',borderRadius:20,...S}}>-{disc}% OFF</span>}
        </div>

        {/* Right details */}
        <div className="pc4-qv-right" style={{flex:1,padding:'36px 32px',display:'flex',flexDirection:'column'}}>
          <div style={{fontSize:9,fontWeight:800,letterSpacing:'.16em',textTransform:'uppercase',color:B.faint,marginBottom:8,...S}}>
            {product.category}
          </div>
          ...
          <div style={{display:'flex',alignItems:'center',gap:3,marginBottom:18}}>
            ...
            <span style={{fontSize:13,fontWeight:700,color:B.body,marginLeft:5,...S}}>{(product.average_rating||4).toFixed(1)}</span>
            <span style={{fontSize:12,color:B.faint,...S}}>({product.total_reviews||0} reviews)</span>
          </div>

          <div style={{display:'flex',alignItems:'baseline',gap:10,flexWrap:'wrap',marginBottom:6}}>
            <span style={{fontSize:30,fontWeight:800,background:B.brandGrad,
              WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text',...S}}>
              {fmtINR(price)}
            </span>
            {orig>price&&<span style={{textDecoration:'line-through',color:B.faint,fontSize:16,...S}}>{fmtINR(orig)}</span>}
            {disc>0&&<span style={{background:B.discGrad,color:'#fff',fontSize:9,fontWeight:800,
              padding:'2px 7px',borderRadius:20,letterSpacing:'.08em',...S}}>-{disc}%</span>}
          </div>
          {saved>0&&<p style={{fontSize:12,color:'#16A34A',fontWeight:600,marginBottom:20,...S}}>🎉 You save {fmtINR(saved)} on this order</p>}
          ...
          <div style={{display:'flex',gap:24,marginBottom:22,flexWrap:'wrap'}}>
            {[
              {label:'Category', val: <span style={{background:tc,color:'#fff',fontSize:10,fontWeight:700,padding:'3px 10px',borderRadius:20,...S}}>{product.category}</span>},
              {label:'SKU',      val: <span style={{fontSize:13,fontWeight:600,color:B.body,...S}}>PRD-{String(product.product_id).padStart(5,'0')}</span>},
              {label:'Stock',    val: stock===0
                ? <span style={{fontSize:12,fontWeight:600,color:'#dc2626',...S}}>Out of Stock</span>
                : <span style={{fontSize:12,fontWeight:600,color:B.green,display:'flex',alignItems:'center',gap:4,...S}}>
              ...
              <div key={label}>
                <div style={{fontSize:9,fontWeight:800,letterSpacing:'.14em',textTransform:'uppercase',color:B.faint,marginBottom:4,...S}}>{label}</div>
                {val}
              </div>
            ))}
          </div>
          ...
          <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:22}}>
            <span style={{fontSize:9,fontWeight:800,letterSpacing:'.14em',textTransform:'uppercase',color:B.faint,...S}}>Qty</span>
            <div style={{display:'flex',alignItems:'center',border:'1px solid #E5E7EB',borderRadius:24}}>
              ...
              <span style={{width:36,textAlign:'center',fontSize:14,fontWeight:600,...S}}>{qty}</span>
          ...
          <div style={{display:'flex',gap:12,marginBottom:16}}>
            <button onClick={handleAdd} disabled={adding||busy==='cart'||stock===0}
              style={{flex:1,height:48,background:cartAdded?B.green:B.brandGrad,border:'none',borderRadius:24,color:'#fff',
                fontSize:13,fontWeight:700,letterSpacing:'.04em',textTransform:'uppercase',
                cursor:(adding||stock===0)?'not-allowed':'pointer',
                opacity:(adding||busy==='cart'||stock===0)?.7:1,
                display:'flex',alignItems:'center',justifyContent:'center',gap:8,transition:'all .2s',...S}}>
              {cartAdded?'✓ Added to Cart':adding?'…':stock===0?'Out of Stock':'🛍 Add to Cart'}
            </button>
            <button onClick={onWish} disabled={busy==='wishlist'}
              style={{width:48,height:48,flexShrink:0,borderRadius:'50%',
                border:`1.5px solid ${wished?B.red:'#E5E7EB'}`,background:wished?'#FFF0F0':'transparent',
                display:'flex',alignItems:'center',justifyContent:'center',
                cursor:busy==='wishlist'?'not-allowed':'pointer',opacity:busy==='wishlist'?.6:1,transition:'all .2s'}}>
          ...
          <Link to={`/product-details/${product.product_id}`} onClick={onClose}
            style={{textAlign:'center',fontSize:12,color:B.faint,textDecoration:'underline',textUnderlineOffset:3,...S}}
...
function ProductCard({ product }: { product: ApiProduct }) {
  ...
  return(
    <>
      {qv&&<QuickViewModal product={product} wished={wished} busy={busy} cartAdded={cartAdded}
        onClose={()=>setQv(false)} onWish={()=>handleWish()} onCart={(qty)=>handleCart(undefined,qty)}/>}
      ...
      <div className="pc4-card"
        ...
        <div className="pc4-img-wrap">
          ...
          {disc>0&&<span className="pc4-disc-badge">-{disc}% OFF</span>}
            ...
            <div className="pc4-ov-btns">
              ...
              <button type="button" className="pc4-ob" onClick={e=>handleWish(e)} disabled={busy==='wishlist'} aria-label="Wishlist">
                <svg width="14" height="14" viewBox="0 0 24 24"
                  fill={wished?'#FCA5A5':'none'} stroke={wished?'#FCA5A5':'#fff'} strokeWidth="2">
                  <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/>
                </svg>
        ...
        <div className="pc4-info">
          ...
          <div className="pc4-stars-row">
            ...
            <span style={{fontSize:12,fontWeight:700,color:B.body,marginLeft:3,fontFamily:FONT_BODY,lineHeight:1}}>
          ...
          <div className="pc4-price-row">
            ...
            <button type="button"
              ...
              {cartAdded
                ...
                :<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={B.muted} strokeWidth="2">
                  <path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/>
                  <line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 01-8 0"/>
                </svg>}
            </button>
          ...
          {/* Mobile cart btn */}
          <button type="button" className={`pc4-mob-cart${cartAdded?' added':''}`}
            ...
            {cartAdded
              ...
              :<><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/>
                  <line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 01-8 0"/>
                </svg>{stock===0?'Out of Stock':'Add to Cart'}</>}
          </button>
        </div>
...
export default function ProductCollection() {
  ...
  const fetchProducts=useCallback(async()=>{
    ...
    }catch(e:any){if(alive.current)setError(e?.message??'Failed to load products.')}
        ...
        <div style={{maxWidth:1720,margin:'0 auto'}}>
          ...
          {!error&&(
            <div className="pc4-grid">
              {loading&&Array.from({length:PAGE_SIZE}).map((_,i)=><SkeletonCard key={`sk-${i}`}/>)}
              {!loading&&displayed.map(p=><ProductCard key={p.product_id} product={p}/>)}
              {loadingMore&&Array.from({length:4}).map((_,i)=><SkeletonCard key={`skm-${i}`}/>)}
            </div>
          )}
          {!loading&&!error&&hasMore&&<div ref={sentinelRef} className="pc4-sentinel" aria-hidden="true"/>}
          {loadingMore&&(
            <div className="pc4-spinner-wrap"><div className="pc4-spinner"/><span>Loading more products…</span></div>
          )}
          {!loading&&!error&&!hasMore&&displayed.length>0&&(
            <div className="pc4-end">
              <div className="pc4-end-rule">
                <div className="pc4-end-dash"/><span>You've seen all {displayed.length} products</span><div className="pc4-end-dash"/>
              </div>
            </div>
          )}
        </div>
